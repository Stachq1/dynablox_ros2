...
VoxbloxMeshVisual::~VoxbloxMeshVisual() {
  ...
  for (const auto& ogre_object_pair : object_map_) {
    scene_manager_->destroyManualObject(ogre_object_pair.second);
  }
}

void VoxbloxMeshVisual::setMessage(const voxblox_msgs::msg::Mesh::SharedPtr msg) {
  for (const auto& mesh_block : msg->mesh_blocks) {
    ...
    for (size_t i = 0; i < mesh_block.x.size(); ++i) {
      const float mesh_x = (static_cast<float>(mesh_block.x[i]) * point_conv_factor + static_cast<float>(index[0])) * msg->block_edge_length;
      const float mesh_y = (static_cast<float>(mesh_block.y[i]) * point_conv_factor + static_cast<float>(index[1])) * msg->block_edge_length;
      const float mesh_z = (static_cast<float>(mesh_block.z[i]) * point_conv_factor + static_cast<float>(index[2])) * msg->block_edge_length;
    ...
    for (size_t i = 0; i < mesh_block.x.size(); ++i) {
      ...
      if (has_color) {
        ...
        color.r = std::numeric_limits<uint8_t>::max() * (mesh.normals[i].x() * 0.5f + 0.5f);
        color.g = std::numeric_limits<uint8_t>::max() * (mesh.normals[i].y() * 0.5f + 0.5f);
        color.b = std::numeric_limits<uint8_t>::max() * (mesh.normals[i].z() * 0.5f + 0.5f);
      }
    ...
    auto it = object_map_.find(index);
    ...
    for (const auto index : connected_mesh.indices) {
